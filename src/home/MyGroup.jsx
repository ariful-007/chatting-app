...
const MyGroup = () => {
  ...
  return (
    <div id="all-item">
      ...
      {groupList.length == 0 ? (
        <h1 className=" text-xl text-black font-bold text-center mt-3">
          No Group available
        </h1>
      ) : showRequest ? (
        <div className=" bg-blue-600 rounded-lg p-10 relative">
          <button
            ...
            className="Button_v_3 absolute w-auto right-1 top-1"
          >
            close
          </button>
          {groupJoinRequest.map((item, i) => {
            return (
              <div
                key={i}
                className="flex justify-between items-center  bg-slate-200 p-3 rounded-lg mt-5"
              >
                ...
                <button
                  onClick={() => handelGroupAccept(item)}
                ...
                <button
                  onClick={() => handelGroupReject(item)}
                  className=" Button_v_3"
                >
                  {" "}
                  Decline{" "}
          ...
          {groupMember.map((item, i) => {
            return (
              ...
              >
                ...
                <button
                  onClick={() => handelGroupMembersDelete(item)}
                  className=" Button_v_3"
                >
                  {" "}
                  Decline{" "}
        ...
        groupList.map((item, i) => {
          return (
            <div key={i} className="flex justify-between items-center p-2">
              ...
              <button
                onClick={() => handelGroupRequest(item)}
                className=" Button_v_3"
              >
                Request
              </button>
